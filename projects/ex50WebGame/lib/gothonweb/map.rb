...
CENTRAL_CORRIDOR = Room.new("Central Corridor",
    ...
    Armory and about to pull a weapon to blast you. What do you do?
    """)
    ...
    get the bomb.  The code is 3 digits. You need to guess it:
    """)
    ...
    hint = ""
    if good_pod < 3
      hint = "[it's less than 3]"
    elsif good_pod > 3
      hint = "[it's larger than 3]"
    end
    ...
    pod_desc = pod_desc + hint
    
    ESCAPE_POD = Room.new("Escape Pod", pod_desc)
    ...
    ESCAPE_POD.add_paths({
      "#{good_pod}" => THE_END_WINNER, #It's very important to use double quotes here, otherwise string interpolation doesn't work
      '*' => THE_END_LOSER
    })
